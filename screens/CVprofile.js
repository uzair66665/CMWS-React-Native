...
global.vid
  ...
  function CheckEmpty() {
    if (Name == null || Address == null || Contact ==null) {
      Alert.alert('Name or Address or Contact Number are empty...!');
        ...
        Customer();
        }else if(global.Usertype == 'Vendor'){
          Vendor();
        }
        else{
        Transporter();
          ...
          .then(data => {
            if(data=="Sorry")
            {
              Alert.alert("Number Already Exists")
                ...
                navigation.navigate('SetLocation',{type:'customerlogin'})
            }
           });
          ...
          .then(data => {
            if(data=="Sorry")
            {
              Alert.alert("Number Already Exists")
                ...
                //navigation.navigate('Vendor_p')
                navigation.navigate('SetLocation',{type:'vendor'})
            }
           });
  ...
  const Transporter=  async() => {
    try {
      console.log('Saving Credintials...')
      
        await  fetch(global.ip+"login/Add_Transporter", {
          method: 'POST',
          headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json'
